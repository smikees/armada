...
def _today(realm, realm_root, today) -> str:
    ...
    d = f"{datefmt.DOW[today.weekday()]} {today.day} {datefmt.MON[today.month - 1]}"   # §9a, locale-proof
    return (f'<div class="mc-widget" style="height:100%">'
...
_ADD_SECTION_MODAL = (
    ...
    f'<label class="mc-label" style="margin-top:0">Name {_STAR}</label>'
    f'<input id="as-name" placeholder="E.g. Daily Digest" class="mc-field">'
    f'<label class="mc-label">Source (file path in the realm, or https URL) {_STAR}</label>'
    f'<input id="as-src" placeholder="E.g. shared/digest.html  ·  or  ·  https://digest.stamih.com" class="mc-field">'
...
from ..assets import js as _js  # noqa: E402
_WIZ_JS = _js("newrealm_wizard")           # static/js/newrealm_wizard.js (UI audit L1)
