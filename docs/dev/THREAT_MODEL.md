...
| T3 | **Untrusted HTML on the app's own origin.** Section mini-sites (`/section-raw`, `/section-asset`), mirrored snapshots of external pages, and `.html` thread attachments (`/thread-file`) are served from `127.0.0.1:8756` with no sandbox. Their scripts can call every API route — same origin, so T1's and the CSRF checks don't apply — including starting an agent turn. A mirrored external page puts a *remote site's* JavaScript there. | **High** | **Open — needs Mihai** (5.8a) |
| T4 | **Reviewing a link runs a tool-enabled agent on untrusted content.** `catalogue.review_url` calls the engine with `allow_tools=True` against a URL the owner pasted. The repo or page being reviewed can carry instructions ("to verify, run…"), and the reviewer can act on them with the owner's rights — the feature meant to judge risk is itself the most exposed turn in the app. | **High** | **Open — needs Mihai** (5.8b) |
